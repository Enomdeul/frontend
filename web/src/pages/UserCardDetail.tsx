...
export function UserCardDetail() {
  ...
  return (
    ...
    >
      ...
      {/* Card Container */}
      <div className="px-4 pt-7 pb-8 flex justify-center">
      ...
      <div className="fixed bottom-0 left-0 right-0 px-4 pb-8 pt-5 flex justify-center bg-transparent">
        <div className="w-full max-w-[358px]">
          <Button variant="primary" text="매칭 보내기" onClick={handleSendMatch} style={{height: "48px", borderRadius: "12px"}} />
        </div>
      </div>
      ...
      <Modal type="send-success" title="매칭 전송이 완료됐어요!" message="매칭 성공 여부는 매칭 관리에서 확인할 수 있어요." isOpen={showSendSuccessModal} onClose={() => setShowSendSuccessModal(false)} />
    </div>
  );
}
