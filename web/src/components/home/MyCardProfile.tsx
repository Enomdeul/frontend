...
export function MyCardProfile({data: propData}: MyCardProfileProps) {
  const { data: cardData, isLoading, error } = useMyCard();
  const { data: skillsData } = useSkills();

  // 스킬 ID를 스킬 이름으로 매핑하는 함수
  const getSkillName = (skillId: number): string => {
    if (!skillsData) return "";
    for (const group of skillsData) {
      const skill = group.skills.find((s) => s.skill_id === skillId);
      if (skill) return skill.skill_name;
    }
    return "";
  };
  ...
  if (propData) {
    ...
    const { name, age, organization, jobGroup, introduction, skills, desiredSkills } = cardData.result;
    ...
    data = {
      ...
      desiredSkills: desiredSkills.map((skill) => ({
        skillId: skill.skillId,
        skillName: getSkillName(skill.skillId),
      })),
    };
